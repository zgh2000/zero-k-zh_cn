...
uv run python deploy_zh.py      # 部署汉化
uv run python restore_en.py     # 还原英文
```
...
- 首次部署会自动备份原版英文到 `backup/original/`，之后可随时还原
- Steam 验证游戏文件后会还原为英文，重新运行 `deploy_zh.py` 即可
...
本项目翻译由 AI 辅助完成，使用 MiMo 自动翻译模型生成初始译文，再由人工校对调整。
