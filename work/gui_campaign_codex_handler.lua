...
local function T_cat(catID)
	if zhCN and zhCN.codex_categories and zhCN.codex_categories[catID] then
		return zhCN.codex_categories[catID]
	end
	return nil
end

local function T_codex(id, field)
	if zhCN and zhCN.codex and zhCN.codex[id] then
		local val = zhCN.codex[id][field]
...
local function UpdateCodexEntry(entry, codexText, codexImage, entryButton)
	...
	codexText:SetText((T_codex(entry.id, "text") or entry.text or "") .. "\n ")
		...
		for j = 1, #cat do
			...
			if unlocked then
				local button = Button:New{
					name = entry.id,
					height = 24,
					caption = T_codex(entry.id, 'name') or entry.name,
					align = "left",
					alignPadding = 0.08,
					OnClick = {
						function(self)
