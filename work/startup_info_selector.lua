...
local moduleNamesMap = {
	["Machine Gun"] = "机枪",
	["Field Radar"] = "野战雷达",
	["Ablative Armour Plates"] = "烧蚀装甲板",
	["Autorepair"] = "自修复",
	["Personal Shield"] = "个人护盾",
	["High Density Plating"] = "高密度装甲",
	["Damage Booster"] = "伤害增幅",
	["Advanced Targeting System"] = "高级瞄准系统",
}
	...
	for i=1,#commData.modules do
		...
		for j, modulename in pairs(commData.modules[i]) do
			...
			if chassisModuleDefs[modulename] then
				...
				local substr = moduleNamesMap[moduleDef.humanName] or moduleDef.humanName
...
local function GetCommSelectTemplate(num, data)
	...
	local displayName = commNameMap[data.name] or data.name
	
	local option = {
		name = data.name,
		tooltip = "选择"..displayName..WriteTooltip(commProfileID),
