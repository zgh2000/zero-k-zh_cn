...
local function MakeWinPopup(planetData, bonusObjectiveSuccess, difficulty, extraCodexUnlocks)
	...
	Label:New {
		...
		caption = i18n("victory_on") .. planetData.name .. "!",
...
local function InitializeBriefingWindow(planetInformation)
	...
	Chili.Label:New{
		...
		caption = i18n("planet") .. planetInformation.name,
	...
	local textSize = planetTextHandler.AddEntry(T(planetInformation.name, "extendedText") or planetInformation.infoDisplay.extendedText)
	...
	if planetInformation.tips then
		...
		for i = 1, #tips do
			...
			textSize = planetTextHandler.AddEntry((zhCN and zhCN.planets and zhCN.planets[planetInformation.name] and zhCN.planets[planetInformation.name].tips and zhCN.planets[planetInformation.name].tips[i]) or tips[i].text, tips[i].image)
...
local function SelectPlanet(popupOverlay, planetHandler, planetID, planetData, startable)
	...
	local fluffLabels = {
		...
		Label:New{caption = (zhCN and zhCN.common and zhCN.common.primary_label) or "Primary", objectOverrideFont = Configuration:GetFont(3)},
		...
		Label:New{caption = (zhCN and zhCN.common and zhCN.common.type_label) or "Type", objectOverrideFont = Configuration:GetFont(3)},


		Label:New{caption = (zhCN and zhCN.terrain_types and zhCN.terrain_types[planetData.infoDisplay.terrainType]) or planetData.infoDisplay.terrainType or "<UNKNOWN>", objectOverrideFont = Configuration:GetFont(3)},
	...
	local planetDesc = TextBox:New {
		...
		text = ((startable or Configuration.debugMode) and (T(planetData.name, "text") or planetData.infoDisplay.text)) or (zhCN and zhCN.common and zhCN.common.locked_planet) or "This planet will need to be approached for further study.",
