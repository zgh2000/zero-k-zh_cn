...
local function TranslatePlanetName(name)
	return T(name, "name") or name
end

local function TranslatePlanetText(name, default)
	return T(name, "text") or default
end

local function TranslatePlanetExtendedText(name, default)
	return T(name, "extendedText") or default
end
...
local function SelectPlanet(popupOverlay, planetHandler, planetID, planetData, startable)
	...
	local planetName = string.upper(TranslatePlanetName(planetData.name))
	...
	local planetDesc = TextBox:New {
		...
		bottom = "25%",
		padding = {0, 0, 10, 0},
		text = ((startable or Configuration.debugMode) and TranslatePlanetText(planetData.name, planetData.infoDisplay.text)) or "This planet will need to be approached for further study.",
		objectOverrideFont = Configuration:GetFont(3),
	}
